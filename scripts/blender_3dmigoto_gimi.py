...
# Blender 4.3+ compatibility: mesh.vertex_colors is deprecated in favor of
# mesh.color_attributes. Provide a helper for safe access.
def get_color_attributes(mesh):
    """Get color attributes for the mesh, compatible across Blender versions."""
    if hasattr(mesh, 'color_attributes'):
        return mesh.color_attributes
    return mesh.vertex_colors

def color_attribute_new(mesh, name):
    """Create a new color attribute, compatible across Blender versions."""
    if hasattr(mesh, 'color_attributes'):
        # In Blender 4.3+, color_attributes.new() requires domain and type.
        # Use 'CORNER' domain (Face Corner) and 'BYTE_COLOR' type to match old vertex_colors behavior.
        try:
            return mesh.color_attributes.new(name=name, domain='CORNER', type='BYTE_COLOR')
        except TypeError:
            return mesh.color_attributes.new(name, type='BYTE_COLOR', domain='CORNER')
    return mesh.vertex_colors.new(name=name)

def mesh_update(mesh):
    """Update mesh geometry, compatible across Blender versions."""
    if hasattr(mesh, 'update_tag'):
        # Blender 4.0+: mesh.update() is deprecated in favor of update_tag()
        try:
            mesh.update_tag()
        except Exception:
            mesh.update()
...
# Blender 4.0+ compatibility: mesh.vertex_layers_float and mesh.vertex_layers_int
# have been removed. Use mesh.attributes API instead.

def _has_legacy_vertex_layers(mesh):
    """Returns True if mesh has the legacy vertex_layers_float/int API."""
    return hasattr(mesh, 'vertex_layers_float')

def _vertex_layer_get_or_none(mesh, name, data_type):
    """Get a vertex layer by name and type. Returns None if not found."""
    if _has_legacy_vertex_layers(mesh):
        if data_type == 'FLOAT':
            if name in mesh.vertex_layers_float:
                return mesh.vertex_layers_float[name]
        else:  # INT
            if name in mesh.vertex_layers_int:
                return mesh.vertex_layers_int[name]
        return None
    # Blender 4.0+ path
    attr = mesh.attributes.get(name)
    if attr is not None and attr.data_type == data_type:
        return attr
    return None

def has_vertex_float_layer(mesh, name):
    """Check if a named float vertex layer exists, Blender-version safe."""
    return _vertex_layer_get_or_none(mesh, name, 'FLOAT') is not None

def has_vertex_int_layer(mesh, name):
    """Check if a named int vertex layer exists, Blender-version safe."""
    return _vertex_layer_get_or_none(mesh, name, 'INT') is not None

def new_vertex_float_layer(mesh, name):
    """Create a new float vertex layer, Blender-version safe.
    Returns the layer object that exposes .data[index].value.
    """
    if _has_legacy_vertex_layers(mesh):
        mesh.vertex_layers_float.new(name=name)
        return mesh.vertex_layers_float[name]
    return mesh.attributes.new(name=name, type='FLOAT', domain='POINT')

def new_vertex_int_layer(mesh, name):
    """Create a new int vertex layer, Blender-version safe.
    Returns the layer object that exposes .data[index].value.
    """
    if _has_legacy_vertex_layers(mesh):
        mesh.vertex_layers_int.new(name=name)
        return mesh.vertex_layers_int[name]
    return mesh.attributes.new(name=name, type='INT', domain='POINT')

def get_vertex_float_layer(mesh, name):
    """Get an existing float vertex layer, Blender-version safe."""
    attr = _vertex_layer_get_or_none(mesh, name, 'FLOAT')
    if attr is not None:
        return attr
    if _has_legacy_vertex_layers(mesh):
        return mesh.vertex_layers_float[name]
...
def get_vertex_int_layer(mesh, name):
    """Get an existing int vertex layer, Blender-version safe."""
    attr = _vertex_layer_get_or_none(mesh, name, 'INT')
    if attr is not None:
        return attr
    if _has_legacy_vertex_layers(mesh):
        return mesh.vertex_layers_int[name]
...
def object_to_mesh(obj, context):
    """Get evaluated mesh from an object, compatible across Blender versions."""
    if hasattr(obj, 'evaluated_get') and hasattr(context, 'evaluated_depsgraph_get'):
        # Blender 2.80+
        depsgraph = context.evaluated_depsgraph_get()
        obj_eval = obj.evaluated_get(depsgraph)
        if hasattr(obj_eval, 'to_mesh'):
            # Blender 2.80 - 3.x
            return obj_eval.to_mesh()
        else:
            # Blender 4.0+: use new_from_object or the evaluated object's data directly
            try:
                return bpy.data.meshes.new_from_object(obj_eval)
            except Exception:
                return obj_eval.data.copy()
    else:
        # Blender 2.79
        return obj.to_mesh(context.scene, True, 'PREVIEW', calc_tessface=False)
...
_3DMIGOTO_TEMP_NORMALS_KEY = "_3dmigoto_temp_normals"
...
def import_normals_step1(mesh, data):
    ...
    # Blender 4.0+: create_normals_split() has been removed. Instead of setting
    # normals through loop.normal (which requires a split normal layer), we store
    # per-vertex normals in a temporary mesh custom data attribute. step2 will
    # read them and call normals_split_custom_set() directly.
    if hasattr(mesh, 'create_normals_split'):
        # Blender 2.80 - 3.x path
        mesh.create_normals_split()
        for l in mesh.loops:
            l.normal[:] = normals[l.vertex_index]
    else:
        # Blender 4.0+ path: store per-vertex normals for step2 to consume.
        # We store as a flat list on a custom mesh attribute that survives validate().
        mesh[_3DMIGOTO_TEMP_NORMALS_KEY] = tuple(n for v in normals for n in v)

def import_normals_step2(mesh):
    # Build per-loop custom normals from the per-vertex data.
    # Blender 4.0+: check if we stored per-vertex normals in step1.
    temp_normals = None
    if _3DMIGOTO_TEMP_NORMALS_KEY in mesh:
        temp_normals = mesh[_3DMIGOTO_TEMP_NORMALS_KEY]
        del mesh[_3DMIGOTO_TEMP_NORMALS_KEY]

    num_loops = len(mesh.loops)
    clnors = array('f', [0.0] * (num_loops * 3))

    if temp_normals is not None:
        # Blender 4.0+ path: build loop normals from stored per-vertex normals,
        # then set them via normals_split_custom_set which works across versions.
        for l in mesh.loops:
            vidx = l.vertex_index * 3
            lidx = l.index * 3
            clnors[lidx]     = temp_normals[vidx]
            clnors[lidx + 1] = temp_normals[vidx + 1]
            clnors[lidx + 2] = temp_normals[vidx + 2]
    else:
        # Blender 2.80 - 3.x path: read back loop normals set in step1
    ...
    # Blender 4.0+: use_auto_smooth has been removed. Setting custom split normals
    # via normals_split_custom_set() is sufficient and will be used by the renderer.
    ...
    for (element_name, data) in sorted(vertex_layers.items()):
        ...
        for component in range(dim):
            ...
            if type(data[0][0]) == int:
                layer = new_vertex_int_layer(mesh, layer_name)
                ...
                layer = new_vertex_float_layer(mesh, layer_name)
                for v in mesh.vertices:
    ...
    for elem in vb.layout:
        ...
        if translated_elem_name == 'POSITION':
            ...
            if len(data[0]) <= 3 or vertex_color_layer_channels == 4:
                ...
                # channel layers
                color_attribute_new(mesh, elem.name)
                color_layer = get_color_attributes(mesh)[elem.name].data
                c = vertex_color_layer_channels
                for l in mesh.loops:
                    color_layer[l.index].color = list(data[l.vertex_index]) + [0]*(c-len(data[l.vertex_index]))
            else:
                color_attribute_new(mesh, elem.name + '.RGB')
                color_attribute_new(mesh, elem.name + '.A')
                color_layer = get_color_attributes(mesh)[elem.name + '.RGB'].data
                alpha_layer = get_color_attributes(mesh)[elem.name + '.A'].data
                for l in mesh.loops:
...
# =============================================================================
# Shared helpers for blender_vertex_to_3dmigoto_vertex*
# These extract common patterns used by both the normal export path and the
# outline export path.
# =============================================================================

def _get_position_w_component(mesh, pos, blender_loop_vertex):
    """Read POSITION.w from the corresponding float vertex layer, if it exists.
    Otherwise pad the position with the default 1.0."""
    if has_vertex_float_layer(mesh, 'POSITION.w'):
        return pos + [get_vertex_float_layer(mesh, 'POSITION.w').data[blender_loop_vertex.vertex_index].value]
    return None

def _get_color_value(mesh, elem, blender_loop_vertex):
    """Read a COLOR value from the mesh's color attributes. Handles both the
    common 'COLOR' attribute and the split COLOR.RGB/COLOR.A layout."""
    color_attrs = get_color_attributes(mesh)
    if elem.name in color_attrs:
        return elem.clip(list(color_attrs[elem.name].data[blender_loop_vertex.index].color))
    try:
        return list(color_attrs[elem.name + '.RGB'].data[blender_loop_vertex.index].color)[:3] + \
               [color_attrs[elem.name + '.A'].data[blender_loop_vertex.index].color[0]]
    except KeyError:
...
def _get_unhandled_semantic(mesh, elem, blender_loop_vertex):
    """Read unhandled vertex-element components from int/float vertex layers.
    Returns a list of values, or an empty list if nothing was found."""
    data = []
    for component in 'xyzw':
        layer_name = '%s.%s' % (elem.name, component)
        if has_vertex_int_layer(mesh, layer_name):
            data.append(get_vertex_int_layer(mesh, layer_name).data[blender_loop_vertex.vertex_index].value)
        elif has_vertex_float_layer(mesh, layer_name):
            data.append(get_vertex_float_layer(mesh, layer_name).data[blender_loop_vertex.vertex_index].value)
    return data

def _get_uvs(elem, blender_loop_vertex, texcoords):
    """Collect uv components (.xy, .zw) for the given TEXCOORD element."""
    uvs = []
...
def blender_vertex_to_3dmigoto_vertex(mesh, obj, blender_loop_vertex, layout, texcoords):
    ...
    for elem in layout:
        ...
        if elem.name == 'POSITION':
            pos_w = _get_position_w_component(mesh, pos, blender_loop_vertex)
            vertex[elem.name] = pos_w if pos_w is not None else elem.pad(pos, 1.0)
        elif elem.name.startswith('COLOR'):
            vertex[elem.name] = _get_color_value(mesh, elem, blender_loop_vertex)
            ...
            vertex[elem.name] = _get_uvs(elem, blender_loop_vertex, texcoords)
        else:
            # Unhandled semantics are saved in vertex layers
            data = _get_unhandled_semantic(mesh, elem, blender_loop_vertex)
            if data:
...
def export_3dmigoto(operator, context, vb_path, ib_path, fmt_path):
    ...
    mesh = object_to_mesh(obj, context)
...
# =============================================================================
# N-Panel (Sidebar panel that exposes the 3DMigoto tools in the 3D Viewport's right sidebar
# (press N in the 3D View to see it).
# =============================================================================

class VIEW3D_PT_3dmigoto_panel(bpy.types.Panel):
    """3DMigoto tools side panel"""
    bl_label = "3DMigoto"
    bl_idname = "VIEW3D_PT_3dmigoto_panel"
    ...
    def draw(self, context):
        ...
        col.separator()
        col.operator(Import3DMigotoFrameAnalysis.bl_idname,
                     text="帧分析转储 (vb.txt + ib.txt)", icon='FILE_FOLDER')
        col.operator(Import3DMigotoRaw.bl_idname,
                     text="原始缓冲 (.vb + .ib)", icon='FILE_BLANK')
        col.operator(Import3DMigotoReferenceInputFormat.bl_idname,
                     text="参考输入格式 (.txt)", icon='TEXT')
        col.operator(Import3DMigotoPose.bl_idname,
        ...
        col.separator()
        col.operator(Export3DMigoto.bl_idname,
                     text="原始缓冲 (.vb + .ib)", icon='FILE_BLANK')
        col.operator(Export3DMigotoGenshin.bl_idname,
        ...
        col.separator()
        col.operator(ApplyVGMap.bl_idname,
                     text="应用顶点组映射 (.vgmap)", icon='GROUP_VERTEX')
        col.operator(UpdateVGMap.bl_idname,
                     text="更新顶点组映射", icon='GROUP_VERTEX')
        col.operator(Merge3DMigotoPose.bl_idname,
                     text="合并姿势", icon='POSE_HLT')
        col.operator(DeleteNonNumericVertexGroups.bl_idname,
